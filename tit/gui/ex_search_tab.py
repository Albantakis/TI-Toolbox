...
def _get_and_display_electrodes(subject_id, cap_name, parent_widget, path_manager=None):
    ...
    if not subject_id:
        QtWidgets.QMessageBox.warning(
            parent_widget, "No Subject", "Please select a subject first"
        )
        return False

    if not cap_name:
        QtWidgets.QMessageBox.warning(
            parent_widget, "No Cap Name", "No EEG cap name provided"
        )
        return False
    ...
    try:
        ...
        electrode_dialog = ElectrodeDisplayDialog(
            clean_cap_name, electrodes, parent_widget
        )
        ...
        QtWidgets.QMessageBox.critical(
            parent_widget, "Error", f"Error reading electrode information: {str(e)}"
        )
        return False
        ...
        try:
            ...
            if not m2m_dir or not os.path.isdir(m2m_dir):
                self.error_signal.emit(
                    f"m2m directory not found for subject {self.subject_id}"
                )
            ...
            net_name = (
                self.eeg_net_file.replace(".csv", "")
                if self.eeg_net_file.endswith(".csv")
                else self.eeg_net_file
            )
            ...
            if os.path.exists(hdf5_path):
                ...
                self.error_signal.emit(
                    "Delete existing file first or choose a different EEG net"
                )
                self.finished_signal.emit(False, "")
                return
            ...
            try:
                ...
                if self.terminated:
                    self.output_signal.emit(
                        "Leadfield generation was terminated", "warning"
                    )
                    self.finished_signal.emit(False, "")
                    return
...
class ExSearchTab(QtWidgets.QWidget):
    ...
    def log_exsearch_start(self, subject_id, roi_count):
        ...
        self.update_output(
            f"Beginning ex-search optimization for subject: {subject_id} ({roi_count} ROI(s))",
            "info",
        )
    ...
    def log_roi_start(self, roi_index, total_rois, roi_name):
        ...
        self.update_output(
            f"├─ Processing ROI {roi_index + 1}/{total_rois}: {roi_name}", "info"
        )
    ...
    def log_step_complete(self, step_name, success=True):
        ...
        if start_time:
            ...
            self.update_output(
                f"├─ {step_name}: {status} ({duration})",
                "success" if success else "error",
            )
        else:
            status = "✓ Complete" if success else "✗ Failed"
            self.update_output(
                f"├─ {step_name}: {status}", "success" if success else "error"
            )
    ...
    def log_roi_complete(self, roi_index, total_rois, roi_name):
        ...
        if start_time:
            duration = self.format_duration(start_time)
            self.update_output(
                f"├─ ROI {roi_index + 1}/{total_rois} ({roi_name}): ✓ Complete ({duration})",
                "success",
            )
        else:
            self.update_output(
                f"├─ ROI {roi_index + 1}/{total_rois} ({roi_name}): ✓ Complete",
                "success",
            )
    ...
    def log_optimization_run_start(self, run_number, total_runs):
        ...
        self.update_output(
            f"├─ Optimization run {run_number}/{total_runs}: Started", "info"
        )
    ...
    def log_optimization_run_complete(self, run_number, total_runs, duration):
        ...
        self.update_output(
            f"├─ Optimization run {run_number}/{total_runs}: ✓ Complete ({duration})",
            "success",
        )
    ...
    def log_exsearch_complete(self, subject_id, total_rois, output_dir):
        ...
        if self.EXSEARCH_START_TIME:
            ...
            self.update_output(
                f"└─ Ex-search optimization completed successfully for subject: {subject_id} ({total_rois} ROI(s), Total: {total_duration})",
                "success",
            )
            self.update_output(f"Results available in: {output_dir}", "success")
        else:
            self.update_output(
                f"└─ Ex-search optimization completed successfully for subject: {subject_id} ({total_rois} ROI(s))",
                "success",
            )
            self.update_output(f"Results available in: {output_dir}", "success")
    ...
    def log_pipeline_configuration(
        self, subject_id, project_dir, selected_net_name, selected_hdf5_path, env
    ):
        ...
        try:
            ...
            config_logger = logging_util.get_file_only_logger(
                "Ex-Search-Config-File-Only", log_file
            )
            ...
            config_logger.info(
                f"Project Name: {os.environ.get('PROJECT_DIR_NAME', 'Unknown')}"
            )
            ...
            if self.use_all_combinations:
                ...
                config_logger.info(
                    f"  Search Strategy: All valid 4-electrode assignments"
                )
                config_logger.info(
                    f"  Constraint: Each electrode used at most once per montage"
                )
                config_logger.info(
                    f"  Valid Combinations: {valid_combos:,} (C({len(self.e1_plus)},4) × 4!)"
                )
                config_logger.info(
                    f"  Formula: Choose 4 from {len(self.e1_plus)}, then assign to positions"
                )
            else:
                ...
                config_logger.info(
                    f"  E1- electrodes ({len(self.e1_minus)}): {', '.join(self.e1_minus)}"
                )
                config_logger.info(
                    f"  E2+ electrodes ({len(self.e2_plus)}): {', '.join(self.e2_plus)}"
                )
                config_logger.info(
    ...
    def log_roi_configuration(self, current_roi, roi_name, x, y, z, env):
        ...
        try:
            ...
            roi_logger = logging_util.get_file_only_logger(
                "Ex-Search-ROI-File-Only", log_file
            )
            ...
            roi_logger.info(
                f"Output Directory: {roi_name}_{env.get('SELECTED_EEG_NET', 'unknown')}"
            )
            ...
            roi_logger.info(
                f"  1. TI Simulation → {roi_name}_{env.get('SELECTED_EEG_NET', 'unknown')}/"
            )
            roi_logger.info(
                f"  2. ROI Analysis → Extract TImax/TImean and generate final CSV"
            )
            roi_logger.info("-" * 60)
        ...
        try:
            ...
            if os.path.exists(log_dir):
                ex_search_logs = [
                    ...
                    if f.startswith("ex_search_") and f.endswith(".log")
                ]
                if ex_search_logs:
                    ...
                    completion_logger = logging_util.get_file_only_logger(
                        "Ex-Search-Completion-File-Only", latest_log
                    )
                    ...
                    completion_logger.info(
                        f"Total ROIs Processed: {len(self.roi_processing_queue)}"
                    )
                    ...
                    if self.roi_processing_queue:
                        ...
                        for i, roi_file in enumerate(self.roi_processing_queue):
                            ...
                            try:
                                ...
                                if selected_items and selected_items[0].data(
                                    QtCore.Qt.UserRole
                                ):
    ...
    def setup_ui(self):
        ...
        self.rb_bucketed.setToolTip(
            "Search electrode combinations within separate E1+/-, E2+/- groups"
        )
        self.rb_all_combinations = QtWidgets.QRadioButton("All Combinations Mode")
        self.rb_all_combinations.setToolTip(
            "Search all possible electrode assignments (any electrode can be in any position)"
        )
        ...
        self.rb_all_combinations.toggled.connect(
        ...
        self.roi_radius_spinbox.setToolTip(
            "Radius of spherical ROI for field extraction (default: 3.0mm)"
        )
        ...
        self.leadfield_list.setSelectionMode(
        ...
        self.selected_leadfield_label.setStyleSheet(
        ...
        self.show_electrodes_leadfield_btn.clicked.connect(
            self.show_electrodes_for_selected_leadfield
        )
        ...
        self.leadfield_list.itemSelectionChanged.connect(
            self.on_leadfield_selection_changed
        )
        ...
        self.action_buttons = RunStopButtons(
            self, run_text="Run Ex-Search", stop_text="Stop Ex-Search"
        )
    ...
    def _build_bucketed_panel(self):
        ...
        self.e1_minus_input.setToolTip(
            "Electrodes for Channel 1 negative pole (cathodes)"
        )
        ...
        self.e2_minus_input.setToolTip(
            "Electrodes for Channel 2 negative pole (cathodes)"
        )
    ...
    def initial_setup(self):
        ...
        try:
            ...
            if self.debug_mode:
                ...
                self.update_output(
                    f"- Subjects with leadfields: {len(subjects_with_leadfields)}"
                )
                ...
                subjects_without_leadfield = [
                    sid for sid in subjects if sid not in subjects_with_leadfields
                ]
                if subjects_without_leadfield:
                    self.update_output(
                        f"- Subjects without leadfields: {', '.join(subjects_without_leadfield)}"
                    )
                ...
                self.update_output(
                    "3. Enter electrodes for each category (E1+, E1-, E2+, E2-)"
                )
    ...
    def refresh_leadfields(self):
        ...
        try:
            ...
            for net_name, hdf5_path, file_size in leadfields:
                ...
                item.setData(
                    QtCore.Qt.UserRole, {"net_name": net_name, "hdf5_path": hdf5_path}
                )
    ...
    def create_leadfield_with_net(self, subject_id, eeg_net_file):
        ...
        if self.leadfield_generating:
            QtWidgets.QMessageBox.warning(
                self, "Already Running", "Leadfield generation is already running"
            )
            return
        ...
        if not project_dir or not os.path.exists(project_dir):
            QtWidgets.QMessageBox.warning(
                self, "No Project", "Project directory not found"
            )
            return
        ...
        self.leadfield_thread = LeadfieldGenerationThread(
            subject_id, eeg_net_file, project_dir, self
        )
        self.leadfield_thread.output_signal.connect(self.update_output)
        self.leadfield_thread.error_signal.connect(
            lambda msg: self.update_output(msg, "error")
        )
        self.leadfield_thread.finished_signal.connect(
            self.leadfield_generation_finished
        )
    ...
    def leadfield_generation_finished(self, success, hdf5_path):
        ...
        if success:
            ...
            self.update_output("=" * 60, "default")
            self.update_output(
                "Leadfield generation completed successfully!", "success"
            )
            self.update_output("=" * 60, "default")
    ...
    def show_electrodes_for_selected_leadfield(self):
        ...
        if not selected_items or not selected_items[0].data(QtCore.Qt.UserRole):
            QtWidgets.QMessageBox.warning(
                self, "No Selection", "Please select a leadfield first"
            )
            return
    ...
    def validate_inputs(self):
        ...
        if self.rb_all_combinations.isChecked():
            ...
            all_electrodes = self.parse_electrode_input(
                self.all_electrodes_input.text()
            )
            ...
            if len(all_electrodes) < 4:
                self.update_status(
                    "At least 4 electrodes are required for all-combinations mode",
                    error=True,
                )
                return False
        else:
            ...
            if not all([e1_plus, e1_minus, e2_plus, e2_minus]):
                self.update_status(
                    "Please enter valid electrode names for all categories", error=True
                )
                return False
    ...
    def run_optimization(self):
        ...
        if self.rb_all_combinations.isChecked():
            ...
            if len(all_electrodes) != len(set(all_electrodes)):
                self.update_status(
                    "Error: Duplicate electrodes found. Each electrode must be unique.",
                    error=True,
                )
                return
        ...
        if self.use_all_combinations:
            ...
            n_combos = (
                len(list(combinations(range(len(e1_plus)), 4))) * 24
            )  # C(n,4) * 4!
            electrode_info += (
                f"Search Space: ~{n_combos:,} unique 4-electrode combinations"
            )
        else:
    ...
    def run_roi_pipeline(self, subject_id, project_dir, ex_search_dir, env):
        ...
        if self.debug_mode:
            ...
            self.update_output(
                f"[DEBUG] Subject: {subject_id} | Project: {project_dir}", "debug"
            )
        ...
        if os.path.exists(roi_output_dir) and os.listdir(roi_output_dir):
            if not confirm_overwrite(
                self, roi_output_dir, f"ROI search directory '{output_dir_name}'"
            ):
                # Skip to next ROI
                self.current_roi_index += 1
                self.run_roi_pipeline(subject_id, project_dir, ex_search_dir, env)
    ...
    def run_current_roi_analysis(self, subject_id, project_dir, ex_search_dir, env):
        ...
        if self.debug_mode:
            ...
            self.update_output(
                f"[DEBUG] ROI analyzer script will run with ROI_LIST_FILE limited to: {current_roi}",
                "debug",
            )
    ...
    def roi_analysis_completed(
        ...
        self.cleanup_and_run_mesh_processing(
            subject_id, project_dir, ex_search_dir, env, temp_roi_list
        )

    def cleanup_and_run_mesh_processing(
        ...
        try:
            if self.debug_mode:
                self.update_output(
                    f"[DEBUG] Removing temp ROI list: {temp_roi_list}", "debug"
                )
    ...
    def run_current_roi_mesh_processing(
        self, subject_id, project_dir, ex_search_dir, env
    ):
    ...
    def disable_controls(self):
        ...
        if self.leadfield_generating:
            self.status_label.setText(
                "Generating leadfield... Stop button and Debug Mode are available"
            )
        else:
            self.status_label.setText(
...
class EEGNetSelectionDialog(QtWidgets.QDialog):
    ...
    def setup_ui(self):
        ...
        desc_label = QtWidgets.QLabel(
            "Choose an EEG net configuration for the new leadfield matrix:"
        )
        ...
        # Default info
        if default_item:
            default_info = QtWidgets.QLabel(
                "Default: GSN-HydroCel-185.csv is pre-selected"
            )
            default_info.setStyleSheet(
                "color: #4caf50; font-style: italic; margin: 5px 0;"
            )
    ...
    def show_electrodes(self):
        ...
        if not selected_items:
            QtWidgets.QMessageBox.warning(
                self, "No Selection", "Please select an EEG net first"
            )
            return
        ...
        if self.parent and hasattr(self.parent, "pm"):
            pm = self.parent.pm
        _get_and_display_electrodes(
            self.subject_id, selected_net, self, path_manager=pm
        )
    ...
    def accept(self):
        ...
        if not self.net_list.selectedItems():
            QtWidgets.QMessageBox.warning(
                self, "No Selection", "Please select an EEG net"
            )
            return
...
class ElectrodeDisplayDialog(QtWidgets.QDialog):
    ...
    def setup_ui(self):
        ...
        self.electrode_list.setSelectionMode(
            QtWidgets.QAbstractItemView.ExtendedSelection
        )

        # Populate electrode list
        for electrode in sorted(
            self.electrodes, key=lambda x: int(x[1:]) if x[1:].isdigit() else 0
        ):
    ...
    def copy_selected_electrodes(self):
        ...
        if not selected_items:
            QtWidgets.QMessageBox.information(
                self, "No Selection", "Please select electrodes to copy"
            )
            return
